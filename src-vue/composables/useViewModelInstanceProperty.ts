...
export interface UseViewModelInstancePropertyOptions<P extends ViewModelInstanceValue, V, R, E = undefined> {
...
export function useViewModelInstanceProperty<P extends ViewModelInstanceValue, V, R, E = undefined>(
  ...
  const safePropertyAccess = (callback: (prop: P) => void) => {
    ...
    // If no instance available but we have executeWhenReady, queue the operation
